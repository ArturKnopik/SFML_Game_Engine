...
public:
	    ...
	    m_topLeft(topLeft), m_size(size), m_level(level)
	{
		...
		m_rectShape.setFillColor(
		    sf::Color(0, static_cast<sf::Uint8>(250 - (level * 15)), 0 + static_cast<sf::Uint8>((level * 15)), 10));
	...
	void split()
	{
		...
		m_topLeftLeaf =
		    std::make_shared<KOD::QuadTreeNode<TObj>>(sf::Vector2f(m_topLeft.x, m_topLeft.y), newSize, newLevel);
		m_topRightLeaf = std::make_shared<KOD::QuadTreeNode<TObj>>(sf::Vector2f{m_topLeft.x + newSize.x, m_topLeft.y},
		                                                           newSize, newLevel);
		m_bottomLeftLeaf = std::make_shared<KOD::QuadTreeNode<TObj>>(sf::Vector2f{m_topLeft.x, m_topLeft.y + newSize.y},
		                                                             newSize, newLevel);
		m_bottomRightLeaf = std::make_shared<KOD::QuadTreeNode<TObj>>(
		    sf::Vector2f{m_topLeft.x + newSize.x, m_topLeft.y + newSize.y}, newSize, newLevel);
	...
	{
		bool isOverlapingX =
		    (bb.m_position.x + bb.m_size.x >= m_topLeft.x && bb.m_position.x <= m_topLeft.x + m_size.x);

		bool isOverlapingY =
		    (bb.m_position.y + bb.m_size.y >= m_topLeft.y && bb.m_position.y <= m_topLeft.y + m_size.y);
		return isOverlapingX && isOverlapingY;
	}
	...
	void addGameObject(std::shared_ptr<TObj> obj)
	{
		...
		if (m_objectLsit.size() < m_maxObjects && m_isSplited == false && m_isSplited == false ||
		    m_level == m_maxDeph) {
	...
	void getAllUniqueObjects(KOD::BoundingBox bb, std::unordered_map<size_t, std::shared_ptr<TObj>>& objMap)
	{
		if (m_isSplited) {
			if (m_topLeftLeaf) {
				if (m_topLeftLeaf->isOverlapping(bb)) {
					m_topLeftLeaf->getAllUniqueObjects(bb, objMap);
				}
			}
			if (m_topRightLeaf) {
				if (m_topRightLeaf->isOverlapping(bb)) {
					m_topRightLeaf->getAllUniqueObjects(bb, objMap);
				}
			}
			if (m_bottomLeftLeaf) {
				if (m_bottomLeftLeaf->isOverlapping(bb)) {
					m_bottomLeftLeaf->getAllUniqueObjects(bb, objMap);
				}
			}
			if (m_bottomRightLeaf) {
				if (m_bottomRightLeaf->isOverlapping(bb)) {
					m_bottomRightLeaf->getAllUniqueObjects(bb, objMap);
				}
			}
		} else {
			for (auto& it : m_objectLsit) {
				objMap.insert({it->getUid(), it});
			}
		}
	}
};
...
class QuadTree
{
	...
	std::unordered_map<size_t, std::shared_ptr<TObj>> m_allObjInBB;
...
public:
	...
	QuadTree(sf::Vector2f topLeft, sf::Vector2f size, size_t level = 0) :
	    ...
	    m_size(size),
	    m_level(level),
	    m_qTreeNode(std::make_shared<QuadTreeNode<TObj>>(topLeft, size, level))
	{}
	virtual ~QuadTree() = default;
	void clear() { m_qTreeNode = std::make_shared<QuadTreeNode<TObj>>(m_topLeft, m_size, m_level); }
	...
	void drawQT(sf::RenderWindow& window) { m_qTreeNode->drawNode(window); }

	size_t getObjectCount() { return m_qTreeNode->getObjectCount(); }
	...
	std::vector <std::shared_ptr<KOD::QuadTree<TObj>>> getAllActiveNodes()
	{
	    ...
	    if (this->m_isSplited == true)
	    {
	        ...
	        if (m_bottomLeftLeaf)
	        {
	            KOD::addVectors<std::shared_ptr<KOD::QuadTree<TObj>>>(nodeVector,
	m_bottomLeftLeaf->getAllActiveNodes());
	        }
	        if (m_bottomRightLeaf)
	        {
	            KOD::addVectors<std::shared_ptr<KOD::QuadTree<TObj>>>(nodeVector,
	m_bottomRightLeaf->getAllActiveNodes());
	        }
	    }
	    else
	    {

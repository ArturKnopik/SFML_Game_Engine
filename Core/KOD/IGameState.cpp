	...
	const std::shared_ptr<IGameObject> gameObject)
{
	...
	return collsionDetectionAABBPosByPos(scanAreaPosition,
		windowAndOffsetSize, gameObject->getPosition(),
		sf::Vector2f(100, 100)); //TODO: fix hardcoded size
}
...
void KOD::IGameState::draw()
{
	...
	for (auto& obj : m_globalGameObjectMap) {
		...
		{
			objDrawable->setPosition(obj.second->getPosition());
			objDrawable->draw(m_game.lock()->m_window);
		}
	}
}

void KOD::IGameState::update(const size_t dt)
{
...
const size_t KOD::IGameState::getLocalAreaCount() const
{
	return m_localObjectMap.size();
}

const size_t KOD::IGameState::getGlobalAreaCount() const
{
	return m_globalGameObjectMap.size();
}
